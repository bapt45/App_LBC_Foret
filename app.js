(function () {
    ...
    const STORAGE_KEY = 'inventaireForestierV23';
    const LEGACY_STORAGE_KEYS = ['inventaireForestierV22', 'inventaireForestierV21', 'inventaireForestierV2'];
    ...
    function renderSummary() {
        ...
        if (rows.length === 0) {
            ...
            tr.innerHTML = '<td colspan="17" class="empty-state">Aucun tènement enregistré pour le moment.</td>';
            ...
            rows.forEach((row, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td data-label="Durée passée">${formatDuration(row.durationTotalMinutes || 0)}</td>
    ...
    function renderTotals() {
        ...
        els.sumDuration.textContent = formatDuration(totalDurationMinutes);
    ...
    function saveStand() {
        ...
        if (row.counted <= 0) {
            showToast('Compte au moins un plant avant d’enregistrer.');
            return;
        }
        ...
        renderAll();
        persist();
        showToast('Tènement enregistré.');
        vibrate(40);
    }

    function newStand() {
        if ((state.alive + state.dead) > 0 && !confirm('Remettre les compteurs du tènement en cours à zéro ?')) return;
        resetCurrentCountersAndName();
        renderAll();
        persist();
        showToast('Nouveau tènement prêt.');
    }
    ...
    function exportCsv() {
        ...
        state.stands.forEach((row, index) => {
            lines.push([
                ...
                formatDuration(row.durationTotalMinutes || 0),
